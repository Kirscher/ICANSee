...
def main():
    ...
    with open(results_csv, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "seed","lr","lr_backbone","lr_head","focal_alpha","focal_gamma","freeze_epochs","sampling","experiment_dir","status"
        ])

        for (seed, lr, lr_bb, lr_hd, fa, fg, fe, samp, ep) in itertools.product(
            seeds, lrs, lr_backbones, lr_heads, focal_alphas, focal_gammas, freeze_epochs, sampling, epochs
        ):
            cmd = [
                "python3", "train.py",
                "--data_path", data_path,
                ...
                "--epochs", str(ep),
                "--lr", str(lr),
                "--lr_backbone", str(lr_bb),
                "--lr_head", str(lr_hd),
                "--freeze_backbone_epochs", str(fe),
                "--loss", "focal_loss",
                "--focal_alpha", str(fa),
                "--focal_gamma", str(fg),
                "--sampling", samp,
            ...
            writer.writerow([seed, lr, lr_bb, lr_hd, fa, fg, fe, samp, "", "ok" if ok else "fail"])
            f.flush()
